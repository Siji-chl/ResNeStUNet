...
class ConvNormLReLU(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, stride=None):
        ...
        if stride is not None:
            ...
            self.conv = nn.Conv3d(input_channels, output_channels, kernel_size=kernel_size, stride=1,
                                  padding=[(i - 1) // 2 for i in kernel_size])
...
class ResidualBlock(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride=None):
        ...
        self.conv1 = nn.Conv3d(in_planes, out_planes, kernel_size=kernel_size, stride=stride,
                               padding=[(i - 1) // 2 for i in kernel_size])
        ...
        self.conv2 = nn.Conv3d(out_planes, out_planes, kernel_size=kernel_size, stride=1,
                               padding=[(i - 1) // 2 for i in kernel_size])
        ...
        if (self.stride is not None and any((i != 1 for i in self.stride))) or (in_planes != out_planes):
            stride_ = stride if stride is not None else 1
            self.downsample_skip = nn.Sequential(nn.Conv3d(in_planes, out_planes, kernel_size=1, stride=stride_,
                                                           bias=False),
